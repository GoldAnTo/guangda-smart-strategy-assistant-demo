      ...
      <div class="result-area">
        ...
        <template v-if="activeMode === 'map'">
          <section class="result-section card map-section">
            ...
            <div class="map-toolbar">
              <div class="axis-label">横轴：风险等级（波动率 →）</div>
              ...
              <div class="axis-label axis-label-v">纵轴：年化收益（↑）</div>
            </div>
            ...
            <div class="shelf-grid">
              ...
              >
                ...
                <div class="shelf-cat-metrics">
                  <div class="scm-item">
                    <div class="scm-val" :class="cat.avgReturn >= 0 ? 'gain' : 'loss'">{{ fmt(cat.avgReturn) }}%</div>
        ...
        <template v-if="activeMode === 'ai'">
          ...
          <section v-else class="result-section card ai-empty-card">
            ...
            <div class="ai-empty-sub">根据左侧客户画像，点击上方「生成 AI 组合建议」按钮，AI 将基于 {{ allStrategies.length }} 条策略自动构建最优组合方案</div>
...
const filteredStrategies = computed(() => {
  const riskLevel = clientProfile.value.risk
  return allStrategies.value.filter(s => (s as any).riskLevel === riskLevel)
})
...
const shelfAnalysis = computed(() => {
  ...
  return Object.entries(catMap).map(([name, strategies]) => {
    ...
    const avgReturn = valid.length ? (valid.reduce((a, b) => a + b.annualReturn, 0) / valid.length) : 0
    ...
    if (name === '债券增强' && avgReturn < 3) gap = '收益偏低，建议升级'

    return {
      name,
      count: strategies.length,
      avgReturn: valid.length ? avgReturn.toFixed(1) : '—',
      avgDrawdown,
      gap,
    }
  })
...
function renderMap() {
  ...
  const strategies = allStrategies.value
  ...
  // 用实际数据范围设置坐标轴
  const volValues = strategies.map(s => s.volatilityValue || 10)
  const retValues = strategies.map(s => s.annualReturn)
  const minVol = 0
  ...
  mapChartInstance.value.setOption({
    ...
    xAxis: {
      ...
      markLine: {
        silent: true,
        lineStyle: { color: 'rgba(23,55,91,0.12)', type: 'dashed', width: 1 },
        data: [
          { xAxis: 5, label: { show: false } },
          { xAxis: 10, label: { show: false } },
          { xAxis: 15, label: { show: false } },
          { xAxis: 20, label: { show: false } },
        ],
      },
    },
    yAxis: {
      ...
      markLine: {
        silent: true,
        lineStyle: { color: 'rgba(23,55,91,0.1)', type: 'dashed', width: 1 },
        data: [{ yAxis: 0 }],
      },
    },
    series: [{
      ...
      symbolSize: (val: number[]) => {
        const dd = val[2]
        return Math.max(40, Math.min(72, 66 - dd * 2))
      },
      data,
      label: {
        show: true,
...
async function runAiDiagnosis() {
  ...
  await new Promise(r => setTimeout(r, 1200)) // 模拟AI思考

  const strategies = filteredStrategies.value.length >= 3
    ? filteredStrategies.value
    : allStrategies.value.slice(0, 6)

  // 入选策略：取该风险类别下年化收益最高的前3-4条
  const picked = [...strategies]
  ...
  const layers = [
    {
      ...
      icon: '⚡',
      color: '#d0680a',
      allocation: picked.length >= 2 ? '40%' : '—',
      strategyName: picked[0]?.name || '—',
      role: '收益贡献核心',
      annualReturn: picked[0] ? `${picked[0].annualReturn >= 0 ? '+' : ''}${picked[0].annualReturn.toFixed(2)}%` : '—',
      drawdown: picked[0] ? `${picked[0].maxDrawdown?.toFixed(1) || '—'}%` : '—',
      winRate: picked[0] ? `${picked[0].winRate.toFixed(0)}%` : '—',
      reason: `高风险高回报定位，在组合中承担收益弹性职能。${picked[0]?.navCategory || ''}策略，${picked[0]?.logicSummary?.slice(0, 30) || ''}。`,
    },
    {
      name: '防御层',
      icon: '🛡️',
      color: '#3a7fbf',
      allocation: picked.length >= 2 ? '40%' : '—',
      strategyName: picked[1]?.name || picked[0]?.name || '—',
      role: '回撤控制核心',
      annualReturn: picked[1] ? `${picked[1].annualReturn >= 0 ? '+' : ''}${picked[1].annualReturn.toFixed(2)}%` : '—',
      drawdown: picked[1] ? `${picked[1].maxDrawdown?.toFixed(1) || '—'}%` : '—',
      winRate: picked[1] ? `${picked[1].winRate.toFixed(0)}%` : '—',
      reason: `低波动低回撤定位，与进攻层形成负相关缓冲。有效降低组合整体波动，在市场下行期提供防御。`,
    },
    {
      name: '流动性层',
      icon: '💧',
      color: '#58c7ff',
      allocation: picked.length >= 3 ? '20%' : picked.length >= 2 ? '20%' : '—',
      strategyName: picked[2]?.name || picked[0]?.name || '—',
      role: '流动性保障',
      annualReturn: picked[2] ? `${picked[2].annualReturn >= 0 ? '+' : ''}${picked[2].annualReturn.toFixed(2)}%` : picked[0] ? `${picked[0].annualReturn >= 0 ? '+' : ''}${picked[0].annualReturn.toFixed(2)}%` : '—',
      drawdown: picked[2] ? `${picked[2].maxDrawdown?.toFixed(1) || '—'}%` : picked[0] ? `${picked[0].maxDrawdown?.toFixed(1) || '—'}%` : '—',
      winRate: picked[2] ? `${picked[2].winRate.toFixed(0)}%` : picked[0] ? `${picked[0].winRate.toFixed(0)}%` : '—',
      reason: `高流动性定位，满足客户日常资金调配需求，同时提供组合的安全垫缓冲。`,
    },
  ]
  ...
  aiResult.value = {
    ...
    synergyLogic: `本组合通过"进攻层"与"防御层"的负相关性设计，实现收益与回撤的解耦管理。进攻层在市场上行时贡献超额收益，防御层在市场调整时提供净值保护，流动性层随时待命。三层分工明确、互相咬合，在客户所要求的${riskLabel}风险框架内，最大化风险调整后收益。组合夏普比率${(avgReturn / (avgDrawdown || 1)).toFixed(2)}，胜率${picked.reduce((a, b) => a + b.winRate, 0) / picked.length.toFixed(0)}%，历史最大回撤控制在${avgDrawdown.toFixed(1)}%以内。`,
  }
...
async function loadStrategies() {
  ...
  try {
    ...
    selectedIds.value = [...allStrategies.value]
      .sort((a, b) => b.annualReturn - a.annualReturn)
      .slice(0, 8)
      .map(s => s.seed)
  } finally {
...
.pick-check.active { background: #6abf40; border-color: #6abf40; color: #fff; }
